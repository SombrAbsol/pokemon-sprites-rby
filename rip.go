...
func ripBatchFilename(filename string) error {
	...
	var things = []struct {
		fn      func(rip *gsc.Ripper, n int, form string, outname string) error
		dirname string
		ext     string
		enabled bool
	}{
	...
	for _, form := range gsc.UnownForms {
		for _, t := range things {
			if !t.enabled {
				continue
			}
...
func ripAnimation(rip *gsc.Ripper, number int, form string, outname string) error {
	var g *gif.GIF
	var err error
	if number == 201 && form != "" {
...
func ripPokemon(rip *gsc.Ripper, number int, form string, outname string) error {
...
func ripPokemonBack(rip *gsc.Ripper, number int, form string, outname string) error {
	var m *image.Paletted
	var err error
	if number == 201 && form != "" {
...
func ripShinyPokemon(rip *gsc.Ripper, number int, form string, outname string) error {
	var m *image.Paletted
	var err error
	if number == 201 && form != "" {
		m, err = rip.Unown(form)
...
func ripShinyPokemonBack(rip *gsc.Ripper, number int, form string, outname string) error {
	var m *image.Paletted
	var err error
	if number == 201 && form != "" {
		m, err = rip.UnownBack(form)
...
func ripShinyAnimation(rip *gsc.Ripper, number int, form string, outname string) error {
	var g *gif.GIF
	var err error
	if number == 201 && form != "" {
		g, err = rip.UnownAnimation(form)
